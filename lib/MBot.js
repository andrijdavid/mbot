...
export default class MBot extends Base {
    ...
    start(port, beforeStart) {
        this._initWebhook();
        if (typeof beforeStart == 'function')
            beforeStart(this.app);
    ...
    loadAllModule(moduleDir = "modules") {
        ...
        const normalizedPath = moduleDir;
        fs.readdirSync(normalizedPath).forEach((file) => {
            this.module(require(path.join(normalizedPath, file)));
        });
    }
    ...
    _exposes(port) {
        let opts = {};
        if (this.subdomain)
            opts.subdomain = this.subdomain.toLowerCase();
    ...
    sendAudioMessage(recipientId, message, options) {
        if (message instanceof Message)
            return this.sendMessage(recipientId, message, options);
        else
            throw new Error('Build your message using AudioMessageBuilder');
    }

    sendVideoMessage(recipientId, message, options) {
        if (message instanceof Message)
            return this.sendMessage(recipientId, message, options);
        else
            throw new Error('Build your message using VidepMessageBuilder');
    }

    sendFileMessage(recipientId, message, options) {
        if (message instanceof Message)
            return this.sendMessage(recipientId, message, options);
        else
            throw new Error('Build your message using FileMessageBuilder');
    }

    sendAttachmentMessage(recipientId, message, options) {
        if (message instanceof Message)
            return this.sendMessage(recipientId, message, options);
        else
            throw new Error('Build your message using AttachmentMessageBuilder');
    }

    sendImageMessage(recipientId, message, options) {
        if (message instanceof Message)
            return this.sendMessage(recipientId, message, options);
        else
            throw new Error('Build your message using ImageMessageBuilder');
    }
    ...
    sendTextMessage(recipientId, text, quickReplies, options) {
        ...
        this.sendTypingIndicator(recipientId, this.typinginterval);
        return this.sendMessage(recipientId, message, options);
    }
    ...
    sendMessage(recipientId, message, options) {
        ...
        if ((Helpers.isCallable(message.isText) && message.isText()) || (message.hasOwnProperty('text') && message.text.length > 500)) {
            ...
            if(Helpers.isNotEmpty(fmsg))
                msg.push({text: fmsg});
            return this.sendTypingIndicator(recipientId, this.typinginterval * 4)
                .then(() => this.__sendMultipleMessageOnce(recipientId, msg, options))
                .catch(err => console.error(err));
        } else {
    ...
    __sendMultipleMessageOnce(recipientId, message, options) {
        if (Array.isArray(message)) {
            let chain = this.sendTypingIndicator(recipientId, this.typinginterval);
            for (let i = 0; i < message.length; i++) {
                chain = chain.then(() => {
                    console.log('Promise ', i);
                    return this.__sendOneMessage(recipientId, message[i], options);
                })
                    .then(() => this.sendTypingIndicator(recipientId, this.typinginterval * 2));

            }
            return chain.catch(err => console.error(err));
        }
        return this.__sendOneMessage(recipientId, message, options);
    }

    __sendOneMessage(recipientId, message, options) {
        if(message.hasOwnProperty('text') && message.text.length > 600)
            throw new Error('Got text message with more than 600 characters. Aborting.');
    ...
    sendTypingIndicator(recipientId, milliseconds) {
        ...
        return new Promise((resolve, reject) => {
            return this.sendAction(recipientId, 'typing_on').then(() => {
                setTimeout(() => this.sendAction(recipientId, 'typing_off')
    ...
    say(recipientId, message, options) {
        if (Array.isArray(message)) {
            let chain = this.sendTypingIndicator(recipientId, this.typinginterval);
            for (let i = 0; i < message.length; i++) {
                chain = chain.then(() => this.__sayOne(recipientId, message[i], options));
            }
            return chain.catch((err) => console.error(err));
        }
        else {
    ...
    __sayOne(recipientId, message, options) {
        ...
        if (typeof message === 'string') {
            ...
            if ((message.quick_replies && message.quick_replies.length > 0)) {
                return this.sendTextMessage(recipientId, message.text, message.quick_replies, options);
            ...
            return this.sendAttachment(recipientId, message.attachment, message.url, message.quick_replies, options);
        }
    ...
    _initWebhook() {
        ...
        this.app.get('/privacy', (req, res) => {
            res.render('privacy', {name: this.name});
        });
        this.app.get('/tos', (req, res) => {
            res.render('tos', {name: this.name});
        });
    }
