    ...
    __sendMultipleMessageOnce(recipientId, message, options) {
        if (Array.isArray(message)) {
            ...
            for (let i = message.length; i >= 0; i--) {
                if(i == message.length)
                    chain = this.__sendOneMessage(recipientId, message[i], options).then(this.sendTypingIndicator(recipientId,this.typinginterval*2));
                else
                    chain = chain.then(this.__sendOneMessage(recipientId, message[i], options).then(this.sendTypingIndicator(recipientId,this.typinginterval*2)));
            }
            return chain;
        }
        return this.__sendOneMessage(recipientId, message, options);
