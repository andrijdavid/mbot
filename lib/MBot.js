...
class MBot extends EventEmitter {
    ...
    sendMessage(recipientId, message, options) {
        ...
        return this.sendTypingIndicator(recipientId, timeout).then(() => this.__sendMultipleMessageOnce(recipientId, msg, options))
    }

    __sendMultipleMessageOnce(recipientId, message, options){
        if(Array.isArray(message))
            message.forEach((msg) => {
                this.__sendOneMessage(recipientId, msg, options);
            });
    }
