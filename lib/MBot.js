...
class MBot extends Base {
    ...
    sendMessage(recipientId, message, options) {
        ...
        if ((Helpers.isCallable(message.isText) && message.isText()) || (message.hasOwnProperty('text') && message.text.length > 620)) {
            ...
            return this.sendTypingIndicator(recipientId, this.typinginterval * 4).then(() => this.__sendMultipleMessageOnce(recipientId, msg, options)).catch(err => console.error(err));
        } else {
