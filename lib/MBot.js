...
class MBot extends EventEmitter {
  ...
  constructor(options) {
    ...
    get responsePreprocessor() {
        return this._responsePreprocessor;
    }

    set responsePreprocessor(value) {
        this._responsePreprocessor.push(value);
    }
  ...
  _exposes(port){
      const tunnel = lt(port, function (err, tunnel) {
          if (err) {
  ...
  sendTextMessage(recipientId, text, quickReplies, options) {
      if(this.responsePreprocessor.length > 0){
          this.responsePreprocessor.forEach(function(preprocessor){
    ...
    this.sendTypingIndicator(recipientId, 555);
  ...
  say(recipientId, message, options) {
    if (typeof message === 'string') {
      return this.sendTextMessage(recipientId, message.trim(), [], options);
