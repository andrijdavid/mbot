...
//       @hudgins suggestions
    ...
    console.info('Conversation is still active, but there\'s no callback waiting for a response.');
    console.info('This probably means you forgot to call convo.end(); in your last callback.');
    console.info('We\'ll end the convo but this message is probably lost in time and space.');
    return this.end();
  }
  ...
  start() {
    ...
    this.bot.getUserProfile(this.userId).then(data => Object.assign(this.context, data)).catch(err => console.log(err));
    return this;
  }
